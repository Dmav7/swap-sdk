import { SupportedChainId, TradeType } from "../../src";
...
describe("parseQuoteApiData", () => {
  it("should parse exact input trade correctly", () => {
    const result = parseQuoteApiData(
    ...
    expect(
      helpers.serialization(result.slippage.minimumReceived.amount),
    ).toEqual({
      denominator: "201",
      numerator: "380",
    });
    ...
    expect(
      helpers.serialization(result.routes[0].slippage.minimumReceived.amount),
    ).toEqual({
      denominator: "201",
      numerator: "380",
    });
  ...
  it("should output path of trade correctly", () => {
    ...
    expect(helpers.serialization(result.routes[0].path[0].symbol)).toBe(
      mockTokenA.symbol,
    );
    expect(helpers.serialization(result.routes[0].path[1].symbol)).toBe(
      mockTokenC.symbol,
    );
  });
  ...
  it("should parse exact output trade correctly", () => {
    ...
    expect(
      helpers.serialization(result.routes[0].slippage.maximumSold.amount),
    ).toEqual({
      denominator: "200",
      numerator: "101",
    });
