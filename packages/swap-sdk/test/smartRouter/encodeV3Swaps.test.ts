...
import { encodeSingleHopV3Swap, encodeV3Swaps } from '../../src/smartRouter/encodeV3Swaps'
import { usdcToFulV3MultiHopExactOutputTradeRoute, usdcToFulV3MultiHopTradeRoute } from '../fixtures/v3MultiHopTrades'
import { usdcToVvsV3SimpleTradeRoute, vvsToUsdcV3ExactOutputTradeRoute } from '../fixtures/v3Trades'
...
describe('encodeV3Swaps', () => {
  it('encodes usdc to ful multihop route', async () => {
    ...
    expect(encoded).toBe(helpers.useFixture(encoded, __filename, 'encoded-usdc-ful-multi-hop.json'))
  })
  ...
  it('encodes usdc to ful multihop route with exact output', async () => {
    ...
    expect(encoded).toBe(helpers.useFixture(encoded, __filename, 'encoded-usdc-ful-multi-hop-exact-output.json'))
  })
})
...
describe('encodeSingleHopV3Swap', () => {
  ...
  it('encodes vvs to exact output usdc route', async () => {
    ...
    const encoded = encodeSingleHopV3Swap(route, TradeType.EXACT_OUTPUT, false, {
      recipient: '0x0000000000000000000000000000000000000003',
    })
