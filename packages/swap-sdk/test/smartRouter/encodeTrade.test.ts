...
import {
  nativeToVvsExactOutputTrade,
  vvsToNativeV3ExactOutputTrade,
} from "../fixtures/nativeExactOutputTrades";
import {
  nativeToVvsTrade,
  vvsToNativeTrade,
} from "../fixtures/nativeTokenTrades";

jest.mock("../../src/smartRouter/contractInterfaces", () => {
  const originalModule = jest.requireActual(
    "../../src/smartRouter/contractInterfaces",
  );
  return {
    encodeSmartRouterFunctionData: jest.fn((...args) =>
      originalModule.encodeSmartRouterFunctionData(...args),
    ),
  };
});
const spiedEncodeSmartRouterFunctionData =
  encodeSmartRouterFunctionData as jest.Mock;
...
describe("encodeTrade", () => {
  ...
  it("encodes NATIVE to VVS route", async () => {
    ...
    const to = recipient;
    expect(
      JSON.parse(toJSON(spiedEncodeSmartRouterFunctionData.mock.calls[0])),
    ).toEqual(["swapExactTokensForTokens", [amountIn, amountOutMin, path, to]]);
  });
  ...
  it("encodes VVS to NATIVE route", async () => {
...

    {
      ...
      expect(spiedEncodeSmartRouterFunctionData.mock.calls[1]).toEqual([
        "unwrapWETH9",
