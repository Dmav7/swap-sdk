...
import { encodeRefundETH, encodeSweepToken, encodeUnwrapWETH9 } from './encodeUnwrapSweep'
  ...
  const calldata = encodeMulticall(
    [
      ...
      ...(outputCustodyDuringTx
        ? [isOutputNative ? encodeUnwrapWETH9(trade, encodeArgs) : encodeSweepToken(trade, encodeArgs)]
        : []),
      ...(isInputNative && trade.type === TradeType.EXACT_OUTPUT ? [encodeRefundETH()] : []),
    ],
...
function nativeTokenInputAmount(trade: Trade): bigint {
  ...
  if (trade.type === TradeType.EXACT_OUTPUT && trade.slippage.type === 'maximumSold' && trade.slippage.maximumSold) {
