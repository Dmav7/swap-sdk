...
export function encodeV2Swaps(
  ...
  if (tradeType === TradeType.EXACT_INPUT) {
    ...
    const maximumSold = route.slippage?.type === 'maximumSold' ? tokenAmountToWei(route.slippage.maximumSold!) : 0n

    if (route.amountOut.amount.lte(Fraction.ZERO)) {
      throw new Error('encodeV2Swaps: does not support 0 amountOut with EXACT_OUTPUT')
    }
