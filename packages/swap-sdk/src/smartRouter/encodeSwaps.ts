...
export function encodeSwaps(
  ...
  return partitionedRoutes.map((partitionedRoute, i) => {
    const partitionOutputCustodyDuringTx = i !== partitionedRoutes.length - 1 || outputCustodyDuringTx
    if (partitionedRoute.pool.every((p) => p.version.startsWith('V3_'))) {
      if (partitionedRoute.pool.length === 1) {
        return encodeSingleHopV3Swap(partitionedRoute, tradeType, partitionOutputCustodyDuringTx, encodeArgs)
      } else {
        return encodeV3Swaps(partitionedRoute, tradeType, partitionOutputCustodyDuringTx, encodeArgs)
      }
    }

    if (partitionedRoute.pool.every((p) => p.version === 'V2')) {
      return encodeV2Swaps(partitionedRoute, tradeType, partitionOutputCustodyDuringTx, encodeArgs)
    }
