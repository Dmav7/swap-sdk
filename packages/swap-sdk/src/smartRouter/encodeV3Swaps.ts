...
export function encodeV3Swaps(
  ...
  if (tradeType === TradeType.EXACT_INPUT) {
    ...
    const amountInMaximum = route.slippage?.type === 'maximumSold' ? tokenAmountToWei(route.slippage.maximumSold!) : 0n
...
function packV3SwapPathData(route: EncodableTradeRoute, tradeType: TradeType) {
  const { types, values } = reduceRoutePath<{
    ...
    ({ values, types }, step, pool) => {
      ...
      return {
        values: [...values, V3_VERSION_TO_FEE[pool.version], step.tokenTo.address],
