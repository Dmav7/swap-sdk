...
export function partitionRouteByProtocol(route: EncodableTradeRoute) {
  const [partitions] = reduceRoutePath<
    [partitions: Partition[], lastPool?: TradeRoutePool]
  >(
    route,
    ([partitioning, lastPool], step, pool) => {
      ...
      if (
        ...
        (pool.version.startsWith("V3_") &&
          lastPool.version.startsWith("V3_")) ||
        ...
        return [
          [
            ...concludedPartitions,
            {
              ...
              tokenFrom: currentPartition
                ? currentPartition.tokenFrom
                : step.tokenFrom,
  ...
  return partitions.map<EncodableTradeRoute>((partition, i) => {
    ...
    return {
      amountIn: isFirst
        ? route.amountIn
        : tokenAmount(Fraction.ZERO, partition.tokenFrom),
      amountOut: isLast
        ? route.amountOut
        : tokenAmount(Fraction.ZERO, partition.tokenTo),
