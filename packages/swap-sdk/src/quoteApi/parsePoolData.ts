...
export function parsePool(poolData: TradeQuotePoolData) {
  ...
  if (version === 'V2') {
    ...
    const v3Pool: V3Pool = {
      ...
      address,
      version,
      token0Price: new Fraction(ratioX96BN, Q192).shr(token1.decimals).shl(token0.decimals),
      token1Price: new Fraction(Q192, ratioX96BN).shr(token0.decimals).shl(token1.decimals),
